...
//                }
            ...
            $divNode.bind("dragstop", function handleDragStop(event, ui) {
                //refresh side bar
//                var sideLi = $("#chart").html();
//                $("#chart").empty();
//                $("#chart").append(sideLi);
//                if ($("#chart li:last-child").hasClass("ui-draggable-dragging")) {
//                    $("#chart li:last-child").remove();
//                }
//
//                //remove list from side bar when added to org-chart
//                if (removeside_node != "") {
//                    $("#chart #" + removeside_node).remove();
//                    removeside_node = "";
//                }
//
//                /* reload the plugin */
...
//                cutomdata();
    ...
    function detailMouseenter(obj,$nodeDiv) {
        if ($(obj).find(".portlet-body>span").length == 0) {
            var duplicate = $(obj).find(" >span.label_node").clone();
            var label_node=duplicate.find("#userInfoName").text();
            var cardId=duplicate.find("#cardId").val();
//            var generation=duplicate.find("#generation").text();
            var appellation=duplicate.find("#appellation").text();
            var spouseName=duplicate.find("#spouseName").text();
            var span=$('<span>');
            span.text("姓名: "+label_node);
            var span1=$('<span>');
            span1.text("称谓: "+appellation);
            var span4=$('<span>');
            span4.text("世代: "+cardId);
            var span2=$('<span>');
            span2.text("配偶: "+spouseName);
            var span3=$('<span>');
            span3.text("个人简介: ");
            $(obj).find(".portlet-body").prepend(span)
            $(obj).find(span).after(span1)
            $(obj).find(span1).after(span4)
            $(obj).find(span4).after(span2)
            $(obj).find(span2).after(span3)
            
        }
        $(obj).find(".top-news").toggle().parent().css({"z-index":"10066"});
        $(obj).find(".scroller").slimScroll({
		    height: '180px',
		    railColor: '#222',
		    railOpacity: 0.3,
		});
        $(obj).toggle(function(){
    		$(".top-news").addClass("full");
    		var width=$(window).height()-300;
    		$(obj).find("#fullscreen-detail").html("还原")
    		$(obj).find(".slimScrollDiv").css({"height":width+"px"})
    		$(obj).find(".scroller").css({"height":"100%"})
    	},function(){
    		$(".top-news").removeClass("full");
    		$(obj).find("#fullscreen-detail").html("全屏")
    		$(obj).find(".slimScrollDiv").css({"height":"180px"})
    		$(obj).find(".scroller").css({"height":"100%"})
//    		$("#profile-content-relationship").css({"height":""})
    ...
    function detailMouseleave(obj){
    	  $(obj).find(".top-news").toggle().parent().removeAttr("style");
    	  $(".top-news").removeClass("full");
  		  $(obj).find("#fullscreen-detail").html("全屏")
  		  $(obj).find(".slimScrollDiv").css({"height":"180px"})
    	  $(obj).find(".scroller").css({"height":"100%"})
    ...
    function buildNode($node, $appendTo, level, opts) {
        var $table = $("<table id='relationship' cellpadding='0' cellspacing='0' border='0'/>");
        var $tbody = $("<tbody/>");
        ...
        $nodeDiv.append(
                "<div class='opciones'>" +
                "</div>")
            .mouseenter(function(){detailMouseenter(this,$nodeDiv)}).mouseleave(function() {detailMouseleave(this)});
